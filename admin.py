...
from models import User, get_db, Course, Follow, ParticipationRedeem, ParticipationCode, Comment, Nota, Entregas, Actividad
db = get_db()
...
def init_admin(app):
    ...
    if admin == None:
        ...
        admin.add_view(ProtectedView(Nota,db.session))
        admin.add_view(ProtectedView(Entregas,db.session))
        admin.add_view(ProtectedView(Actividad,db.session))
